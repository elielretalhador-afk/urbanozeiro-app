...
export const EventsHub: React.FC<EventsHubProps> = ({
  ...
  const getTypeBadge = (type: EventType) => {
    switch (type) {
      case 'RACE':
        return { label: 'CORRIDA', icon: Flag, color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30' };
      case 'TOURNAMENT':
  ...
  const getStatusBadge = (status: EventStatus, current: number, max: number) => {
    ...
    if (status === 'REGISTRATION_OPEN') {
      return {
        ...
        color: 'text-emerald-400 bg-emerald-500/20 border-emerald-500/40 shadow-[0_0_10px_rgba(0,255,102,0.2)]',
      };
    }
    return {
      label: 'EM BREVE',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0c1420] via-[#091815] to-[#0a101d] border border-emerald-500/30 p-4 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 text-emerald-400 text-xs font-black uppercase tracking-wider font-mono-stat">
          ...
          <div className="hidden sm:flex flex-col items-end gap-1 font-mono-stat">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 text-xs font-bold">
        ...
        <div className="relative mt-3">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-black/40 border border-white/10 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-400 transition-colors"
          />
        </div>
      </div>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl whitespace-nowrap font-bold text-xs transition-all cursor-pointer ${
              selectedTypeFilter === f.id
                ? 'bg-emerald-400 text-black shadow-[0_0_12px_rgba(0,255,102,0.3)]'
        ...
        {filteredEvents.map((ev) => {
          ...
          return (
            <div
              ...
              className="group relative bg-[#0d141f] hover:bg-[#111c2a] border border-white/10 hover:border-emerald-500/40 rounded-2xl p-4 transition-all duration-200 cursor-pointer shadow-lg hover:shadow-[0_4px_25px_rgba(0,255,102,0.15)]"
            >
              ...
              <div className="flex items-center justify-between gap-2 mb-2.5">
                ...
                <div className="flex items-center gap-1.5">
                  {isUserRegistered && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 text-[10px] font-black font-mono-stat">
              ...
              {/* Event Name */}
              <h4 className="text-base font-black text-white group-hover:text-emerald-400 transition-colors uppercase tracking-tight font-display">
                {ev.name}
              </h4>
              ...
              <div className="mt-3 pt-2.5 flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="p-1 rounded-lg bg-white/5 group-hover:bg-emerald-400/20 group-hover:text-emerald-400 text-slate-400 transition-colors">
