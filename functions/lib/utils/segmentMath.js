...
exports.getPathBoundingBox = getPathBoundingBox;
exports.isPointInsideBoundingBox = isPointInsideBoundingBox;
exports.distancePointToLineSegment = distancePointToLineSegment;
exports.getDistanceToPath = getDistanceToPath;
exports.distanceToSegmentStart = distanceToSegmentStart;
exports.distanceToSegmentEnd = distanceToSegmentEnd;
exports.getNearestEndpoint = getNearestEndpoint;
 ...
 */
function getPathBoundingBox(path, paddingMeters) {
 ...
 */
function isPointInsideBoundingBox(point, box) {
    if (!box || !point)
        return false;
    const [lat, lng] = point;
...
function distancePointToLineSegment(point, lineStart, lineEnd) {
    ...
    const projection = [projLat, projLng];
 ...
 */
function getDistanceToPath(point, path) {
    ...
    for (let i = 0; i < path.length - 1; i++) {
        ...
        if (result.distanceMeters < minDistance) {
            ...
            bestResult = Object.assign(Object.assign({}, result), { segmentIndex: i });
        }
    }
    return bestResult;
}
 ...
 */
function distanceToSegmentStart(point, path) {
    if (!path || path.length === 0)
        return null;
 ...
 */
function distanceToSegmentEnd(point, path) {
    if (!path || path.length === 0)
        return null;
 ...
 */
function getNearestEndpoint(point, path, toleranceMeters) {
...
//# sourceMappingURL=segmentMath.js.map