...
export const MAX_REASONABLE_SPEED_KMH = 65; // High speed for urban skating, above this is very suspicious
export const MAX_REASONABLE_ACCELERATION_KMH_S = 15; // 0 to 15 km/h in 1 second is huge for a skater
export const TELEPORT_THRESHOLD_KMH = 100; // If you teleport faster than 100km/h
export const MAX_BAD_ACCURACY_METERS = 100;
...
export function auditTrack(trackPoints: TrackPoint[]): AuditResult {
  ...
  for (let i = 1; i < trackPoints.length; i++) {
    ...
    if (accelerationKmhS > maxAcceleration) maxAcceleration = accelerationKmhS;
    ...
    if (accelerationKmhS > MAX_REASONABLE_ACCELERATION_KMH_S) {
      result.reasons.push('impossible_acceleration');
      result.riskScore += 20;
    }

    if (pt.accuracy && pt.accuracy > MAX_BAD_ACCURACY_METERS) {
      totalBadAccuracy++;
    }
