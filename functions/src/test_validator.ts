import { validateAttempt, TrackPoint } from './validator';

const fakeSegmentPath: [number, number][] = [
...
async function runTests() {
  ...
  let res = validateAttempt(track1, fakeSegmentPath);
  ...
  res = validateAttempt(track2, fakeSegmentPath);
  ...
  const hash1 = validateAttempt(track1, fakeSegmentPath).trackHash;
  const hash2 = validateAttempt(track1, fakeSegmentPath).trackHash;
  console.log(`TESTE 5 (Replay Hash): ${hash1 === hash2 && hash1 !== undefined ? 'PASS' : 'FAIL'} - Hash1: ${hash1}`);

  // TESTE 6: Timestamp regressivo
  const track6: TrackPoint[] = [
  ...
  res = validateAttempt(track6, fakeSegmentPath);
  ...
  const track7: TrackPoint[] = [
    { latitude: -23.550520, longitude: -46.633309, timestamp: 1000000 }
  ];
  res = validateAttempt(track7, fakeSegmentPath);
