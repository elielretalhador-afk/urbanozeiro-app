...
export const FeedView: React.FC<FeedViewProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<ActivityFilterType>('TODAS');
  ...
  const blockedIds = currentUser.blockedPlayers || [];
                  ...
                  <div className="flex items-center gap-2.5 min-w-0 cursor-pointer" onClick={() => !isOwner && onSelectPlayer && onSelectPlayer(act.playerId)}>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm font-bold text-white truncate font-display">{isOwner ? currentUser.nickname : act.playerName || 'Patinador'}</span>
                      </div>
          ...
          <div className="bg-[#0d141e] rounded-t-3xl p-4 h-[90vh] flex flex-col animate-in slide-in-from-bottom-8">
            ...
            <div className="flex gap-3 mb-4">
              ...
              <textarea 
                ...
                className="flex-1 bg-transparent border-none text-white text-base resize-none focus:ring-0 focus:outline-none placeholder:text-slate-500 min-h-[150px]"
              />
            </div>

            <div className="mt-auto border-t border-white/10 pt-4 flex gap-3">
              <button onClick={requestCamera} className="flex items-center gap-2 text-emerald-400 bg-emerald-500/10 px-4 py-2 rounded-xl font-bold text-xs uppercase hover:bg-emerald-500/20">
                <Camera className="w-5 h-5" /> Foto
              </button>
              <button onClick={requestVideo} className="flex items-center gap-2 text-emerald-400 bg-emerald-500/10 px-4 py-2 rounded-xl font-bold text-xs uppercase hover:bg-emerald-500/20">
...
console.log('Fixed FeedView entirely');
